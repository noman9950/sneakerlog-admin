...
import { updateAbout, getAbout } from '../backend/services/aboutService';
...
export default class AboutForm extends React.Component {
  ...
  componentDidMount() {
      ...
      .then((response) => {
        this.setState({
          about: response,
          description: RichTextEditor.createValueFromString(response[0].about, 'html'),
          previousContent: RichTextEditor.createValueFromString(response[0].about, 'html'), 
        });
      });
  }
  ...
  setDescription = (description) => {
    ...
    about[0].about = description.toString('html');
    this.setState({
      about,
      description,
    });
  ...
  postPrivacyPolicy(event) {
    ...
    const { loading, description, about } = this.state;
    if (!loading) {
      const fd = new FormData();

      this.setState({ loading: true });
        let cloneObject = Object.assign({}, about[0])
        updateAbout(about[0].uuid, cloneObject)
          .then((response) => {
            this.setState({
              loading: false,
              showSnackBar: true,
